...
class FooInferenceShapePass
    : public mlir::PassWrapper<FooInferenceShapePass, FunctionPass> {
 public:
  void runOnFunction() override {
    ...
    while (!opWorklist.empty()) {
      ...
      if (auto p = dyn_cast<InferenceShape>(op)) {
        p.inferShapes();
      } else {
        op->emitError(
...
std::unique_ptr<mlir::Pass> createInferenceShapePass() {
  return std::make_unique<FooInferenceShapePass>();
}
}  // namespace foo
}  // namespace mlir

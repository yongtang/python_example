...
int main(int argc, char **argv) {
  ...
  if (int error = mlir::foo::emitMLIR(*module, enableOptimization,
                                      emitMLIRChoice[emitAction], context)) {
    return error;
  }
  ...
  if (emitAction == EmitAction::EmitRunJIT) {
    double args_in[1] = {0.0};
    double args_out[1] = {0.0};
    void *data[2] = {&args_in[0], &args_out[0]};
    llvm::MutableArrayRef<void *> args(data, 2);
    if (int error =
            mlir::foo::runJIT(*module, enableOptimization, "foo", args)) {
      return error;
    }

    llvm::outs() << args_out[0] << "\n";
    return 0;
  }
